...
describe('cipher (SQLCipher) encryption test(s)', function() {
    ...
    var openDatabase = function(first, second, third, fourth, fifth, sixth) {
      ...
      // FUTURE TBD:
      xit(suiteName + ' Attempt to open encrypted DB with INCORRECT password THEN OPEN & READ with CORRECT PASSWORD [PLUGIN BROKEN - BUG #43: MUST CLOSE THEN TRY AGAIN]', function (done) {
      ...
      it(suiteName + 'Attempt to open and read unencrypted DB with password key [TBD Android vs iOS / macOS]', function (done) {
        ...
        openDatabase({name: dbName}, function (db) {
            ...
            db.sqlBatch([
              ...
              db.close(function () {
                window.sqlitePlugin.openDatabase({name: dbName, key: 'test-password', location: 'default'}, function (db2) {
                  // EXPECTED RESULT on Android ONLY:
                  if (!isAndroid) done.fail();
                  // SHOULD NOT BE ABLE TO READ DATA:
                  db2.executeSql('SELECT * FROM tt', null, function(rs) {
                    // NOT EXPECTED:
                    done.fail();
                  }, function (error) {
                    // EXPECTED RESULT:
